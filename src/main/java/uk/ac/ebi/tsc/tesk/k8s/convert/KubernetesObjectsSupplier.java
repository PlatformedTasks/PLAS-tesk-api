...
public class KubernetesObjectsSupplier {
    ...
    public V1Job taskMasterTemplate() {
        try (InputStream inputStream = getClass().getClassLoader().getResourceAsStream("taskmaster.json");
            ...
            containerEnvironment.stream().filter( env -> "TRANSFER_PVC_NAME".equals(env.getName()) ||
                    "CONTAINER_BASE_PATH".equals(env.getName())).forEach( env -> {

                if (env.getName().equals("TRANSFER_PVC_NAME") && env.getValue() != null){
                    V1Volume tmVolume = new V1Volume();
                    tmVolume.setName("transfer-volume");
                    V1PersistentVolumeClaimVolumeSource transferPvc = new V1PersistentVolumeClaimVolumeSource();
                    transferPvc.setClaimName(env.getValue());
                    tmVolume.setPersistentVolumeClaim(transferPvc);
                    job.getSpec().getTemplate().getSpec().addVolumesItem(tmVolume);
                }
                if (env.getName().equals("CONTAINER_BASE_PATH") && env.getValue() != null) {
                    V1VolumeMount tmVolumeMount = new V1VolumeMount();
                    tmVolumeMount.name("transfer-volume");
                    tmVolumeMount.mountPath(env.getValue());
                    job.getSpec().getTemplate().getSpec().getContainers().get(0).addVolumeMountsItem(tmVolumeMount);
                }
            });
